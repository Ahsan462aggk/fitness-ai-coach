...
const Dashboard = () => {
  ...
  // Use lowercase 'router' to match standard conventions
  const router = useRouter(); 

  // FIX 2 & 3: Wrapped fetchUserData in useCallback and handled the 'any' error type
  const fetchUserData = useCallback(async () => {
    try {
      // FIX 1: Changed 'let' to 'const'
      const token = localStorage.getItem('token');
      
      const response = await axios.get<UserData>('https://ahsan462agk-fitness-ai-coach.hf.space/users/profile', {
        headers: {
  ...
  useEffect(() => {
    fetchUserData();
  }, [fetchUserData]); // FIX 3: Added missing dependency
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard icon={<FiActivity className="w-6 h-6" />} title="Current Weight" value={userData.current_weight} />
          <StatCard icon={<FiTarget className="w-6 h-6" />} title="Target Weight" value={userData.target_weight} />
          <StatCard icon={<FiHeart className="w-6 h-6" />} title="Fitness Level" value={userData.expertise} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
              <FiCalendar /> Today&apos;s Status {/* FIX 4: Escaped apostrophe */}
            </h2>
            <div className={`p-4 rounded-lg text-center ${
              ...
              {userData.rest_days.includes(userData.current_day) ? 'Rest Day' : 'Workout Day'}
            </div>
            
            <h3 className="text-lg font-medium mt-6 mb-3">Rest Days</h3>
            <div className="flex flex-wrap gap-2">
              {userData.rest_days.map(day => (
                <span key={day} className="px-3 py-1 bg-gray-100 rounded-full text-sm">
                  {day}
                </span>
              ))}
            </div>
